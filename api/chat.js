...
Be concise and practical.`,
  ...
  'claude-3-5-haiku-20241022': `You are a construction assistant for Rockmud. Help with cost estimates, schedules, and proposals for underground utility work.
...
Be concise and practical.`,
  ...
  'claude-3-5-sonnet-20241022': `You are a construction assistant for Rockmud. Help with cost estimates, schedules, and proposals for underground utility work.
...
Be concise and practical.`,
  ...
  'claude-3-opus-20240229': `You are a construction assistant for Rockmud. Help with cost estimates, schedules, and proposals for underground utility work.
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { messages, model = 'gpt-4o-mini', temperature = 0.7, max_tokens = 512 } = req.body || {};
    ...
    const effectiveModel = isMud1 ? 'gpt-4o-mini' : model;
    ...
    if (isAnthropic) {
      ...
      if (!apiKey) {
        return res.status(500).json({
          ...
          response: 'Anthropic API key is not set. Add ANTHROPIC_API_KEY to your Vercel environment variables.',
        });
      }

      const anthropic = new Anthropic({ apiKey });
      const systemContent = messages[0]?.role === 'system' ? messages[0].content : null;
      const chatMessages = systemContent ? messages.slice(1) : messages;
      const systemPrompt = getSystemPrompt(model);

      const response = await anthropic.messages.create({
        model: effectiveModel,
        max_tokens,
        system: systemContent ? `${systemPrompt}\n\n${systemContent}` : systemPrompt,
    ...
    if (!apiKey) {
      return res.status(500).json({
        ...
        response: 'OpenAI API key is not set. Add OPENAI_API_KEY to your Vercel environment variables.',
      });
    }
    ...
    return res.status(500).json({
      error: msg,
      response: 'Sorry, the assistant could not respond. Check your API keys and try again.',
    });
  }
};
